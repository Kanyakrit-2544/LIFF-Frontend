...
import { AlertTriangle, Check, CircleUserRound, ExternalLink, Link2, LogOut, RefreshCw, ShoppingBag, X } from "lucide-react";
...
function ProfileLink({ customerId, label = "ดูโปรไฟล์ลูกค้า" }: { customerId: string; label?: string }) {
  return <a className="profile-link" href={`/admin/customer/${encodeURIComponent(customerId)}`}><ExternalLink size={15}/>{label}</a>;
}
...
function CustomerLinks({ items }: { items: CustomerLinkReviewItem[] }) {
  ...
  return <div className="review-list">{items.map(({ link, customer, legacyPerson, payments, enrollments, legacyAvailable }) => (
    <article className="review-item" key={link._id}>
      <header><div><span className="item-type">ประวัติลูกค้าเดิม</span><h2>{customer ? displayName(customer) : link.customerId}</h2></div><div className="item-actions"><span className={`confidence ${link.confidence}`}>{link.confidence}</span><ProfileLink customerId={link.customerId}/></div></header>
...
function PartnerItems({ items }: { items: PartnerReviewItem[] }) {
  ...
  return <div className="review-list">{items.map((item) => {
    ...
    return <article className="review-item" key={event._id}>
      <header><div><span className="item-type">{event.partnerId} · {event.type}</span><h2>{event.status === "quarantined" ? "ข้อมูลต้องแก้ไข" : "ยังระบุตัวลูกค้าไม่ได้"}</h2></div><div className="item-actions"><span className="status-badge">{event.status}</span>{event.customerId && <ProfileLink customerId={event.customerId}/>}</div></header>
