  ...
  "events": [
    {
      ...
      "type": "purchase",                       // purchase | purchase.void | intent | intent.void | erase
...
## A11. `type: "erase"` — คำขอลบข้อมูลส่วนบุคคล (PDPA)

ลูกค้าใช้สิทธิ์ขอให้ลบข้อมูล — สิทธิ์นี้ครอบทั้งสองระบบ ระบบ tag ลบฝั่งตัวเองแล้วต้องบอก CRM ด้วย

```jsonc
{
  "eventId": "tagger-erase-000045",
  "type": "erase",
  "occurredAt": "2026-08-29T09:00:00+07:00",
  "subject": { "lineUserId": "Uxxxxxxxx" },     // ⭐ บังคับ ไม่มี = rejected
  "erase": {
    "reason": "customer_request",                // customer_request | legal_request | other
    "requestedAt": "2026-08-29T08:30:00+07:00"
  }
}
```

### ⭐ CRM ทำอะไร — "ตัดตัวตนออกจากธุรกรรม" ไม่ใช่ลบทิ้งหมด

การซื้อขายเป็นเอกสารทางบัญชีที่มีกฎหมายอื่นบังคับให้เก็บ
ถ้าลบ `purchases` ทิ้ง ยอดขายย้อนหลังจะเปลี่ยนและงบการเงินไม่ตรง

| ข้อมูล | ผล |
|---|---|
| ชื่อ · เบอร์ · อีเมล · social · รูป · ปีเกิด · heardFrom | **ลบ** (เป็น null) |
| คำตอบในฟอร์ม LIFF (`customer_profiles`) | **ลบทั้งหมด** — free text อ่อนไหวที่สุด |
| การเชื่อมกับ LINE user (`identities`) | **ลบ** — ทักเข้ามาใหม่นับเป็นลูกค้าใหม่ ซึ่งถูกต้อง |
| ค่าประเมินความสนใจ (`customer_intents`) | **ลบ** — เป็นการทำโปรไฟล์ ไม่ใช่เอกสารบัญชี |
| `purchases` / `purchase_items` | **เก็บไว้** ผูกกับรหัสที่ไม่ชี้ไปหาใครแล้ว |
| สถานะลูกค้า | เปลี่ยนเป็น `erased` + บันทึก `erasedAt` |

ชีต Google และสำเนาใน `line_crm_ai` จะถูกเขียนทับด้วยฉบับที่ลบแล้วโดยอัตโนมัติ (ตั้งธง sync ให้)

### กฎ

**1. ⭐ ระบุตัวไม่ได้ = ไม่ลบ** ไม่เจอ หรือเจอหลายคน → ตอบ `pending_identity` **ห้ามเดา**
ลบผิดคนคือความเสียหายที่กู้คืนไม่ได้ — ต่างจากเคสอื่นที่เดาผิดแล้วแก้ทีหลังได้

**2. ไม่สร้างลูกค้าใหม่** ต่างจาก `purchase`/`intent` ที่สร้างลูกค้าจาก `lineUserId` ที่ยังไม่รู้จัก
`erase` จะไม่สร้าง — ไม่รู้ว่าใครก็ไม่ต้องลบ

**3. ส่งซ้ำได้ปลอดภัย** `eventId` เดิม → `duplicate` เหมือน event ชนิดอื่น

**4. ⚠️ ขอบเขตที่ยังไม่ครอบ** — ประวัติการซื้อในชีตขายเดิม (`line_crm_legacy`)
เป็นระบบเอกสารคนละชุดที่ยังไม่ได้ต่อกับสิทธิ์นี้ ถ้าลูกค้าขอลบต้องจัดการแยกด้วยมือ
**ต้องบอกลูกค้าให้ตรงตามนี้ อย่ารับปากว่าลบหมดทุกที่**
...
|---|---|---|
| `accepted` | บันทึกแล้ว (สำหรับ `erase` = ลบเรียบร้อย) | ไม่ต้องทำอะไร |
...
export interface PartnerEventDoc {
  ...
  revision: number;
  type: "purchase" | "purchase.void" | "intent" | "intent.void" | "erase";
  occurredAt: Date;
  receivedAt: Date;
