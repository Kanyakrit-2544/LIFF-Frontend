...
2. เปลี่ยนชื่อเล่นใน Mongo เป็น "ชายใหม่" + สถานะเป็น customer แล้วตั้ง dirty
3. รอ sync

ผล: R3 ชื่อเล่น=ชายใหม่  สถานะ=customer  หมายเหตุ=พนักงานจดไว้: โทรแล้ว 27/8
     แถวรวม 4 แถวเท่าเดิม
```
✅ อัปเดตในแถวเดิม **ไม่สร้างแถวซ้ำ** และ **ไม่ทับหมายเหตุพนักงาน** — RISK-4 ปิด

**เจอเรื่องน่าสนใจ:** ลูกค้าจริงที่กรอกฟอร์มบน production แสดงเบอร์เป็น `09x-xxx-6154` (masked)
ขณะที่ข้อมูลทดสอบที่สร้างจากเครื่องแสดงเบอร์เต็ม — เพราะ **`PII_KEY` ของ production กับ local คนละตัว**
`toSheetRow()` ถอดรหัสไม่ได้จึง fallback เป็นค่า mask แทนที่จะพังทั้งรอบ (ทำงานตามที่ออกแบบ)

→ **WF-C ต้องชี้ `API_BASE` ไปที่ environment เดียวกับที่เขียนข้อมูล** ไม่งั้นได้แต่ค่า mask
...
- **แท็บ `_Log`** สร้างไว้แล้วแต่ WF-C ยังไม่เขียนลงไป
- **ลบแถวเมื่อลูกค้าถูก merge** — tombstone ยังค้างในชีต ควรเขียนว่า `MERGED → cus_xxx`
- **`_Schema` tab** ไม่ต้องมีแล้ว เพราะไม่มี AI ที่ต้อง match Column ID
